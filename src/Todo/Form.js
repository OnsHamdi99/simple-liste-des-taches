...
const Form = ({ title, setTitle, addItem, editItem, edit, error, setError }) => {
    ...
    const handleChange = (event) => {
        ...
        setTitle(title);
        if(title.length === 0){
            setError("Ajouter un titre à la tâche!");
        }else{
            setError("");
        }
    }
    ...
    const handleClick = () => {
        if(title.length === 0){
            setError("Il faut insérer un titre !");
            return;
        }
        if (edit) {
            editItem();
        ...
        <Container maxWidth="sm" className={classes.root}>
            <Grid container alignItems="center">
                <Grid item md={12}>
                    ...
                    error={!!error} helperText={error} id="outlined-basic" fullWidth label="Insérer une tâche" multiline variant="outlined" />
                </Grid>
                <Grid item md={12}>
